...
encoded_training_sets = flatten(tmpencoder.encode_input(training_sets))
encoded_testing_sets = flatten(tmpencoder.encode_input(testing_sets))
...
for layer_i in xrange(n_layers):  # Training
    ...
    x = computers[layer_i].train(layer_inputs, layer_labels)
    ...
    if layer_i < n_layers - 1:  # No need to test the last layer before the very real test
        ...
        o = np.append(encoded_training_sets, o, axis=1)
...
for layer_i in xrange(n_layers):  # Testing
    ...
    n_correct, n_incorrect = correctness(o,
                                         flatten(testing_labels) if layer_i < n_layers - 1 else final_layer_testing_labels)
    ...
    if layer_i < n_layers - 1:
        ...
        o = np.append(encoded_testing_sets, o, axis=1)
